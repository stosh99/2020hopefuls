...
def tweet_processing_thread():
    while True:
        lst_tweet = tweet_queue.get()
        print('queue size is:  ', tweet_queue.qsize())
        process_tweet(lst_tweet)
        tweet_queue.task_done()


def process_tweet(lst_tweet):
    """
    print(df_tweet['dt_utc'])
    df_tweet['dt_utc'] = [datetime.strftime(df_tweet[x], "%Y-%m-%d %H:%M:%S") for x in df_tweet['dt_utc']]
    df_tweet['dt'] = datetime.strftime(df_tweet['dt'], "%Y-%m-%d %H:%M:%S")
    df_tweet['created_at'] = datetime.strftime(df_tweet['created_at'], "%Y-%m-%d %H:%M:%S")
    """
    lst_dict = []
    for row in lst_tweet:
        tweet_dict = get_candidates(row)
        tweet_dict['sent'] = calc_sentiment(tweet_dict['text'], tweet_dict['full_text'], tweet_dict['q_text'])
        lst_dict.append(tweet_dict)
        df_tweet = pd.DataFrame(lst_dict)
    if df_tweet['coord'] is None:
        df_tweet['coord'] = 'None'
